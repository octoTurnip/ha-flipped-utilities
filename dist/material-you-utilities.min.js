...
Use a query string like ?i=1 (between 0 and 3 inclusive) to choose alternate color options.`,init:{config:{icon:"mdi:image",min:0,max:255}},card:{config:{text:{}},tabBarIndex:2},handler:tO},css_file:{domain:"input_text",default:"",name:"CSS Path/URL",description:"Path/URL to a CSS file to apply to the page and iframe roots for defining custom properties.",init:{config:{icon:"mdi:language-css3",min:0,max:255}},card:{config:{text:{}},tabBarIndex:2},handler:rI}};async function rO(e,t,r){return e.callWS({type:`${t}/create`,...r})}async function rL(e,t,r,a){return e.callWS({type:`${t}/update`,[`${t}_id`]:r,...a})}async function rH(e,t,r){e.callWS({type:`${t}/delete`,[`${t}_id`]:r})}async function rN(e,t){if(!t)return!0;let r=new Event("hass-action",{bubbles:!0,composed:!0});return r.detail={action:"tap",config:{tap_action:{action:"fire-dom-event",confirmation:t}}},e.dispatchEvent(r),new Promise(e=>{let t,r=()=>{clearTimeout(t),window.removeEventListener("ll-custom",a),window.removeEventListener("dialog-closed",i)},a=()=>{r(),e(!0)};window.addEventListener("ll-custom",a);let i=()=>{t=setTimeout(()=>{r(),e(!1)},100)};window.addEventListener("dialog-closed",i)})}async function rV(e){return new Promise(async t=>{let r=(await tA()).hass,a=r.user?.id,i=window.browser_mod?.browserID?.replace(/-/g,"_");if(r.connection.connected&&a){let n=[];for(let e in rF){let t=n.find(t=>t.handler==rF[e].handler);t?t.inputs.push(e):(t={inputs:[e],handler:rF[e].handler},n.push(t))}let o=[];for(let t of n){let n=[];if(e.id?n=[...t.inputs.map(t=>t_(t,e.id))]:(n=[...t.inputs.map(e=>t_(e)),...t.inputs.map(e=>t_(e,a))],i&&n.push(...t.inputs.map(e=>t_(e,i)))),r.user?.is_admin)o.push(r.connection.subscribeMessage(()=>t.handler(e),{type:"subscribe_trigger",trigger:{platform:"state",entity_id:n}},{resubscribe:!0}));else for(let a of n)o.push(r.connection.subscribeMessage(r=>{"error"in r&&(console.error(r.error),tM(r.error)),t.handler(e)},{type:"render_template",template:`{{ states("${a}") }}`,entity_ids:a,report_errors:!0}))}t(Promise.all(o))}else setTimeout(()=>t(rV(e)),100)})}var rU=function(e,t,r,a){var i,n=arguments.length,o=n<3?t:null===a?a=Object.getOwnPropertyDescriptor(t,r):a;if("object"==typeof Reflect&&"function"==typeof Reflect.decorate)o=Reflect.decorate(e,t,r,a);else for(var s=e.length-1;s>=0;s--)(i=e[s])&&(o=(n<3?i(o):n>3?i(t,r,o):i(t,r))||o);return n>3&&o&&Object.defineProperty(t,r,o),o};customElements.get("disk-color-picker")||r(899);let rz=`${tk}-theme`;class rj extends es{constructor(){super(...arguments),this.tabBarIndex=0,this.tabs=["theme","styles","other"],this.unsubscribers=[]}async handleDeleteHelpers(e){if(!await rN(this,{text:"Are you sure you want to delete these helpers?"}))return;for(let e in rF){let t=t_(e,this.dataId);this.hass.states[t]&&await rH(this.hass,rF[e].domain,t.split(".")[1])}let t="Global input entities cleared";if(this.dataId){let e="";e=this.dataId==this.hass.user?.id?this.hass.user?.name??"":this.hass.states[this.dataId]?.attributes.friendly_name??this.dataId,t=`Input entities cleared for ${e}`}t$(this,t)}buildDeleteHelpersButton(){return j`<div class="delete button" @click="${this.handleDeleteHelpers}">Delete Helpers</div>`}async handleCreateHelpers(e){let t="";for(let e in this.dataId&&(t=` ${this.hass.states[this.personEntityId??""]?.attributes?.friendly_name??this.personEntityId??this.dataId}`),rF){let r=t_(e,this.dataId);if(!this.hass.states[r]){let a=r.split(".")[1];await rO(this.hass,rF[e].domain,{name:a,...rF[e].init.config}),await rL(this.hass,rF[e].domain,a,{name:`${tw} ${rF[e].name}${t}`,...rF[e].init.config});let i=rF[e].domain,n=rB[i],o={entity_id:r};switch(i){case"input_text":case"input_number":o.value=rF[e].default;break;case"input_select":o.option=rF[e].default;break;case"input_boolean":n=`turn_${rF[e].default}`}await this.hass.callService(i,n,o)}}let r="Global input entities created";t&&(r=`Input entities created for${t}`),t$(this,r)}buildCreateHelpersButton(){return j`<div class="create button" @click="${this.handleCreateHelpers}">Create Helpers</div>`}handleTabBar(e){let t=this.tabs.indexOf(e.detail.name);this.tabBarIndex!=t&&(this.tabBarIndex=t)}buildTabBar(e,t,r){return j`<sl-tab-group @sl-tab-show="${t}">${r.map((t,r)=>j`<sl-tab slot="nav" panel="${t}" .active="${r==e}">${t}</sl-tab>`)}</sl-tab-group>`}async handleSelectorChange(e){let t=e.target.getAttribute("field"),r=e.detail.value,a=rF[t].domain,i=rB[a],n={entity_id:t_(t,this.dataId)};switch(a){case"input_text":case"input_number":n.value=r||rF[t].default;break;case"input_select":n.option=r||rF[t].default}await this.hass.callService(a,i,n),this.requestUpdate()}buildSelector(e,t,r,a){let i,n=t_(t,this.dataId),o=this.hass.states[n]?.state;return i="input_boolean"===rF[t].domain?"on"==o:o??rF[t].default,j`<ha-selector .hass="${this.hass}" .name="${e}" .selector="${r}" .value="${i??a}" .label="${e}" .placeholder="${a}" .required="${!1}" field="${t}" @value-changed="${this.handleSelectorChange}"></ha-selector>`}async handleKeyDown(e){!e.repeat&&["Enter"," "].includes(e.key)&&(e.preventDefault(),(e.target.parentElement?.className.replace("button","").trim()==="reset"?this.handleResetClick:this.handleMoreInfoClick).call(this,new window.MouseEvent("click",e),e.target))}async handleResetClick(e,t){let r=(e.target??t).getAttribute("field"),a=rF[r].domain,i=rB[a],n={entity_id:t_(r,this.dataId)};switch(a){case"input_text":case"input_number":n.value=rF[r].default;break;case"input_select":n.option=rF[r].default;break;case"input_boolean":i=`turn_${rF[r].default}`}await this.hass.callService(a,i,n),this.requestUpdate()}buildResetButton(e){return j`<div class="reset button"><ha-icon @click="${this.handleResetClick}" @keydown="${this.handleKeyDown}" tabindex="0" field="${e}" .icon="${"mdi:restore"}"></ha-icon></div>`}handleMoreInfoClick(e,t){let r=t_((e.target||t).getAttribute("field"),this.dataId),a=new Event("hass-more-info",{bubbles:!0,cancelable:!0,composed:!0});a.detail={entityId:r},this.dispatchEvent(a)}buildMoreInfoButton(e){let t=t_(e,this.dataId),r=this.hass.states[t]?.attributes.icon;return j`<pre class="description">${rF[e].description}</pre><div class="more-info button"><ha-icon @click="${this.handleMoreInfoClick}" @keydown="${this.handleKeyDown}" tabindex="0" field="${e}" .icon="${r}"></ha-icon></div>`}buildRow(e){let t=t_(e,this.dataId);if(!this.hass.states[t])return"";let r=this.hass.states[t]?.state;if("base_color"==e){let t,a=e=>{clearTimeout(t);let r=e.target,a=r.value;t=setTimeout(()=>{let e=new Event("value-changed");e.detail={value:a},r.dispatchEvent(e)},100)};return j`<div class="column"><disk-color-picker field="${e}" value="${r}" @change="${a}" @keyup="${a}" @value-changed="${this.handleSelectorChange}"></disk-color-picker><div class="subrow"><div class="row">${this.buildMoreInfoButton(e)}<div class="label">${rF[e].name}</div></div><div class="row"><div class="label secondary">${r||rF[e].default}</div>${this.buildResetButton(e)}</div></div></div>`}let a="";"harmonize"==e&&(a=j`<div class="row semantic-colors">${["pink","red","deep-orange","orange","amber","yellow","lime","light-green","green","teal","cyan","light-blue","blue","indigo","deep-purple","purple"].map(e=>j`<div class="semantic-color" style="background:var(--${e}-color)"></div>`)}</div>`);let i=rF[e].card.config;return"input_number"==rF[e].domain?(i.min=this.hass.states[t]?.attributes?.min??-1,i.max=this.hass.states[t]?.attributes?.max??1,i.step=this.hass.states[t]?.attributes?.step??.01):"input_boolean"==rF[e].domain&&(r="on"==r),j`${this.buildMoreInfoButton(e)} ${this.buildSelector(rF[e].name,e,i,r)}${a} ${rF[e].card.resetButton?this.buildResetButton(e):""}`}setupIds(){this.dataId&&!this.personEntityId&&(this.personEntityId=Object.keys(this.hass.states).find(e=>e.startsWith("person.")&&this.hass.states[e].attributes.user_id==this.dataId))}applyThemeMode(){if(this.darkMode!=this.hass.themes?.darkMode){this.darkMode=this.hass.themes?.darkMode;let e=this.shadowRoot?.getElementById(rz);if(e){let t=[];for(let[e,r]of Object.entries(this.hass.themes?.themes[tw].modes?.[this.darkMode?"dark":"light"]??{}))t.push(`--${e}: ${r};`);e.textContent=`
					:host {
						${t.join("\n")}
					}
				`}}}render(){this.setupIds();let e="Global";this.dataId&&(e=this.hass.states[this.personEntityId??""]?.attributes?.friendly_name??this.personEntityId??this.dataId);let t=Object.keys(rF).filter(e=>rF[e].card.tabBarIndex==this.tabBarIndex);this.hass.states[t_("spec",this.dataId)]?.state!="2025"&&(t=t.filter(e=>"platform"!=e));let r={};for(let e of t)r[e]=this.buildRow(e);for(let e in r)r[e]||delete r[e];return j`<ha-card .hass="${this.hass}" .header="${e}">${this.personEntityId?j`<div class="subtitle">ID: ${this.dataId}</div>`:""} ${this.buildTabBar(this.tabBarIndex,this.handleTabBar,this.tabs)}<div class="card-content">${Object.keys(r).length!=t.length?tT(this.hass.user?.is_admin?"Press Create Helpers to create and initialize inputs.":"Some or all input helpers not setup! Ask an Home Assistant administrator to do so.",this.hass.user?.is_admin?"info":"error"):""} ${Object.keys(r).map(e=>j`<div class="row ${e}">${r[e]}</div>`)}</div>${this.hass.user?.is_admin?j`<div class="card-actions">${this.buildCreateHelpersButton()}${this.buildDeleteHelpersButton()}</div>`:""}</ha-card><style id="${rz}"></style>`}firstUpdated(){let e=async()=>{let t=structuredClone(this.hass.themes?.themes[tw]);delete t.modes;let r=Object.entries(t);if(r.length){for(let[e,t]of r)this.style.setProperty(`--${e}`,t);return}setTimeout(()=>e(),100)};e();let t={targets:[this],id:this.dataId??""};for(let e of[tO,tR,r$,rI])e(t)}updated(){this.applyThemeMode();let e=this.shadowRoot?.querySelector("disk-color-picker");if(e&&!e.shadowRoot?.getElementById(rz)){let t=document.createElement("style");t.id=rz,t.textContent=`
			...
			`,e.shadowRoot?.appendChild(t)}}connectedCallback(){super.connectedCallback(),rV({targets:[this],id:this.dataId??""}).then(e=>{this.unsubscribers=e})}disconnectedCallback(){super.disconnectedCallback(),this.unsubscribers.forEach(async e=>{e()})}static get styles(){return l`:host{width:var(--width);font-family:var(--font-family)}.card-content{display:flex;flex-direction:column;padding:16px;margin:0;gap:20px}.subtitle{margin-top:-24px;padding:0 16px 16px;color:var(--secondary-text-color);font-size:var(--md-sys-typescale-label-large-size,14px);font-weight:var(--md-sys-typescale-label-large-weight,500);line-height:var(
					...
					var(--mdc-fab-box-shadow)
						...
						var(--mdc-fab-box-shadow)
					)}.theme-mode-fab:hover::after{opacity:.08}}.theme-mode-fab:active::after,.theme-mode-fab:focus-visible::after{opacity:.18}`}}rZ([eh()],rq.prototype,"hass",void 0),rZ([eh()],rq.prototype,"narrow",void 0),rZ([eh()],rq.prototype,"route",void 0),rZ([eh()],rq.prototype,"panel",void 0),rZ([ed()],rq.prototype,"tabBarIndex",void 0),rZ([ed()],rq.prototype,"darkModeIndex",void 0),async function(){if(window.MaterialYouInit)return;window.MaterialYouInit=!0,rP(window),tE(document.querySelector("html"),`${tw} Utilities v${t.i8}`);let e=await tA();new MutationObserver(async t=>{for(let r of t)for(let t of r.addedNodes)if("IFRAME"==t.nodeName){let t=await tC(e.shadowRoot,"iframe"),r=await tP(t,"contentWindow");rP(r);let a=await tP(r,"document"),i={targets:[await tC(a,"body")]};for(let e of[tR,r$,rI])await e(i)}}).observe(e.shadowRoot,{subtree:!0,childList:!0}),customElements.define(`${tk}-config-card`,rj),customElements.define(`${tk}-panel`,rq);let r=async t=>{if(t>1e4)return;let a=e.hass?.themes?.theme;if(!a)return void setTimeout(()=>{r(t)},2*t);if(a.includes(tw)){let e={targets:[await tC(document,"html")]};for(let t of[tO,tR,r$,rI,rS])await t(e)}};r(100),rV({});let a=async()=>{let e=(await tA()).hass,t=e.user?.id;if(e.connection.connected&&t){e.connection.subscribeEvents(()=>tR({}),"themes_updated"),e.user?.is_admin&&e.connection.subscribeEvents(e=>{e?.data?.service=="set_theme"&&setTimeout(()=>tR({}),1e3)},"call_service");return}setTimeout(()=>a(),100)};a()}()})()})();